...
import { login, register, loginWithGoogle, saveAuth, UserProfile } from '../services/auth';
...
export default function Auth({ onAuthenticated, onToast }: AuthProps) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      onToast('success', 'Signed in with Google', `Welcome, ${result.user.firstName || result.user.email}`);
    } catch (err) {
      const friendlyMsg = formatErrorMessage(err);
      setError(friendlyMsg);
      onToast('error', 'Google authentication failed', friendlyMsg);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(108,59,255,0.16),_transparent_20%),radial-gradient(circle_at_bottom_right,_rgba(25,198,209,0.12),_transparent_35%),linear-gradient(135deg,_#fff9f2_0%,_#fef8f1_45%,_#f7f0ff_100%)] text-[#17152B] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md rounded-[20px] border border-[#DED9EA] bg-white/95 p-8 shadow-[0_24px_80px_rgba(108,59,255,0.12)]">
        ...
        <button
          ...
          disabled={loading}
          className="w-full flex items-center justify-center gap-3 py-3 px-4 bg-white border border-[#DED9EA] rounded-[14px] text-xs font-bold text-[#17152B] hover:bg-[#F4F1FA] transition-colors mb-6 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
        >
        ...
        <div className="relative mb-6 flex items-center justify-center">
          <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-[#DED9EA]" /></div>
          <span className="relative bg-white px-3 text-[10px] font-bold uppercase tracking-wider text-[#6C6880]">or with email</span>
        </div>
